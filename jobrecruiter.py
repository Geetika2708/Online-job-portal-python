...
class jobs:
    def __init__(self, root):
        ...
        lbl_Name = Label(Manage_Frame, text="ID :", bg="royal blue", fg="white", font=("times new roman", 15, "bold"))
        ...
        lbl_company = Label(Manage_Frame, text="company", bg="royal blue", fg="white", font=("times new roman", 15, "bold"))
        ...
        lbl_qualification = Label(Manage_Frame, text="qualification :", bg="royal blue", fg="white",
                            font=("times new roman", 15, "bold"))
        ...
        lbl_jobid = Label(Manage_Frame, text="Job ID :", bg="royal blue", fg="white", font=("times new roman", 15, "bold"))
        ...
        lbl_Address = Label(Manage_Frame, text="job vacancy :", bg="royal blue", fg="white",
                            font=("times new roman", 15, "bold"))
        ...
        lbl_posted = Label(Manage_Frame, text="Last date:", bg="royal blue", fg="white",
                           font=("times new roman", 15, "bold"))
        ...
        Updatebtn = Button(btn_Frame, text="Update", width=10, command=self.update_data).grid(row=0, column=1, padx=10,
                                                                                              pady=5)
        Clearbtn = Button(btn_Frame, text="Clear", width=10, command=self.fetch_d).grid(row=0, column=2, padx=10, pady=5)
    ...
    def register_job(self):
        ...
        cursor = con.cursor()
        cursor.execute("insert into jobs(job,email,gender,qual,jobid,address,job,lastdate) values(%s,%s,%s,%s,%s,%s,%s,%s)", (self.Name_var.get(),
    ...
    def fetch_data(self):
        con = mysql.connector.connect(host="localhost", user="root", passwd="", database='userdata')
        cur = con.cursor()
        cur.execute("select jobtitle,email,gender,qual,jobid,address,job,lastdate,status from Jobregister2")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def update_data(self):
        ...
        cursor = con.cursor()
        cursor.execute(
            "update jobregister2 set Email=%s,Gender=%s,qual=%s,jobid=%s,Address=%s,Job=%s,lastdate=%s where id=%s",
            (self.company_var.get(),
             self.salary_var.get(),
             self.qualification_var.get(),
             self.jobid_var.get(),
    ...
    def fetch_d(self):
        con = mysql.connector.connect(host="localhost", user="root", passwd="", database='userdata')
        cursor = con.cursor()
        cursor.execute("SELECT email FROM jobregister2 WHERE id = %s", (self.Name_var.get(),))
        row = cursor.fetchone()
    ...
    def Search_data(self):
        con = mysql.connector.connect(host="localhost", user="root", passwd="", database='userdata')
        cur = con.cursor()
        cur.execute("select jobtitle,email,gender,qual,jobid,address,job,lastdate,status from Jobregister2 where " + str(self.Search_by.get()) + " LIKE '%" + str(
            self.Search_txt.get()) + "%'")
        rows = cur.fetchall()
        if len(rows) != 0:
            self.Job_table.delete(*self.Job_table.get_children())
