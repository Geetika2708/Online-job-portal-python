    ...
    if genderCombo.get() == '' or usernameEntry.get() == '' or passwordEntry.get() == '' or confirmEntry.get() == '' or contactEntry.get() == '' or qualificationCombo.get() == '' or addressEntry.get('1.0', 'end-1c') == '' or resumeEntry.get() == '':
        ...
        try:
            ...
            if row is not None:
                ...
                query = 'INSERT INTO login2(id, email, username, password, contact, qualification,address, resume) VALUES(%s, %s, %s, %s, %s, %s,%s,%s)'
...
addressEntry = Text(frame, width=29, height=3, font=('Microsoft Yahei UI LIGHT', 10, 'bold'), fg='white', bg='firebrick1')
addressEntry.grid(row=15, column=0, sticky='w', padx=25)
addressEntry.bind('<FocusOut>', validate_address)

resumeLabel = Label(frame, text='Resume', font=('Microsoft Yahei UI LIGHT', 10, 'bold'), bg='white', fg='firebrick1')
resumeLabel.grid(row=16, column=0, sticky='w', padx=25, pady=(10, 0))

resumeEntry = Entry(frame, width=30, font=('Microsoft Yahei UI LIGHT', 10, 'bold'), fg='white', bg='firebrick1')
resumeEntry.grid(row=17, column=0, sticky='w', padx=25)

browseButton = ttk.Button(frame, text='Browse', style='TButton', command=browse_resume)
browseButton.grid(row=17, column=0, sticky='e', padx=25)

check = IntVar()
checkButton = Checkbutton(frame, text='I accept all terms and conditions', variable=check, onvalue=1, offvalue=0, font=('Microsoft Yahei UI LIGHT', 10, 'bold'), bg='white', fg='firebrick1')
checkButton.grid(row=16, column=0, sticky='w', padx=25, pady=(10, 0))

signupButton = Button(frame, text='SIGNUP', font=('Microsoft Yahei UI LIGHT', 10, 'bold'), bg='firebrick1', fg='white', relief=GROOVE, command=connect_database)
signupButton.grid(row=17, column=1, sticky='w', padx=25, pady=(10, 20))

loginButton = Button(frame, text='LOGIN', font=('Microsoft Yahei UI LIGHT', 10, 'bold'), bg='firebrick1', fg='white', relief=GROOVE, command=login_page)
