...
class jobs:
    def __init__(self, root):
        ...
        Manage_Frame.place(x=20, y=100, width=450, height=800)
        ...
        lbl_Qualif = Label(Manage_Frame, text="Qualification:", bg="crimson", fg="white",
                            font=("times new roman", 15, "bold"))
        ...
        lbl_date = Label(Manage_Frame, text="Date:", bg="crimson", fg="white",
                         font=("times new roman", 15, "bold"))
        ...
        Regbtn = Button(btn_Frame, text="Register", width=10, command=self.insert_data).grid(row=0, column=0, padx=10,
                                                                                              pady=5)
        ...
        quitbtn = Button(btn_Frame, text="quit", width=10, command=self.back).grid(row=1, column=2, padx=10, pady=5)
    ...
    def fetch_d(self):
        con = mysql.connector.connect(host="localhost", user="root", passwd="", database='userdata')
        cursor = con.cursor()
        cursor.execute("SELECT username, contact, qualification,email,address FROM login2 WHERE id = %s", (self.Name_var.get(),))
        row = cursor.fetchone()
